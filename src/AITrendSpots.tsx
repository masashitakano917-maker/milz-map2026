...
import { Sparkles, ExternalLink, Bookmark, Heart, Loader as Loader2, TrendingUp } from 'lucide-react';
...
export default function AITrendSpots({ areaKey, locale, userId }: Props) {
  ...
  return (
    <section className="bg-white p-6 md:p-10 xl:p-16 rounded-[2rem] md:rounded-[3rem] border border-stone-100 shadow-sm space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] font-black text-stone-400 uppercase tracking-[0.4em]">{t.eyebrow}</p>
          </div>
          <h3 className="text-2xl md:text-3xl font-black text-black tracking-tight leading-tight max-w-2xl">
            {t.title}
          </h3>
          <p className="text-sm text-stone-500 font-medium max-w-2xl">{t.subtitle}</p>
        </div>
      ...
      <div className="bg-stone-50 border border-stone-200 rounded-2xl px-5 py-4 flex items-start gap-3">
        <Sparkles className="w-4 h-4 mt-0.5 text-stone-500 shrink-0" />
        <p className="text-[11px] md:text-xs text-stone-600 leading-relaxed font-medium">{t.disclaimer}</p>
      </div>
